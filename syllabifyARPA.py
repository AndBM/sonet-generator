...
def syllabifyARPA(arpa_arr, return_list=False, silence_warnings=False):
    ...
    for i in range(len(final_arr)):
        if not testLegalCoda(final_arr[i]):
            handleError(
                "Impossible to syllabify %s according to English "
...
def testLegalOnset(syllable):
    ...
    if length > 3:
        ...
        # s-voiceless_stop-approximant or s-voiceless_fricative-r
            ...
            or (cluster[1] in VOICELESS.intersection(FRICATIVES) and cluster[2] == "R")
        ):
            return cluster[0]
        ...
        if not (
            ...
            or (cluster[0] in STOPS and cluster[1] in APPROXIMANTS)
            or (
                cluster[0] in VOICELESS.intersection(FRICATIVES).union(["V"])
                and cluster[1] in APPROXIMANTS
            )
            or
            ...
            or (cluster[0] == "S" and cluster[1] in NASALS.difference(["NG"]))
            or
            # Other clusters normalized through loanwords, e.g. SH-N, S-V, NW, MR
            (cluster[0] == "SH" and cluster[1] in NASALS and cluster != "NG")
            or (cluster[0] == "S" and cluster[1] == "V")
            or (cluster[0] == "M" and cluster[1] in APPROXIMANTS)
            or (cluster[0] == "N" and cluster[1] == "W")
        ):
            return cluster[0]
...
def testLegalCoda(syllable):
    ...
    # English disallows 5+ length coda clusters
        ...
        if (
            (cluster[3] == "S" and cluster[2] in S_EXTENDED_CODAS)
            or (cluster[3] == "Z" and cluster[2] in Z_EXTENDED_CODAS)
            or (cluster[3] == "T" and cluster[2] in T_EXTENDED_CODAS)
            or (cluster[3] == "D" and cluster[2] in D_EXTENDED_CODAS)
        ):
            length -= 1
        else:
            return False
    ...
    if length == 3:
        if (
            (cluster[2] == "S" and cluster[1] in S_EXTENDED_CODAS)
            or (cluster[2] == "Z" and cluster[1] in Z_EXTENDED_CODAS)
            or (cluster[2] == "T" and cluster[1] in T_EXTENDED_CODAS)
            or (cluster[2] == "D" and cluster[1] in D_EXTENDED_CODAS)
        ):
            length -= 1
        ...
        # 3-phone clusters beginning with L, e.g., Alps, milked
            ...
            or (cluster[1] == "P" and cluster[2] == "S")
            or (cluster[1] == "F" and cluster[2] == "TH")
            or (cluster[1] == "T" and cluster[2] == "S")
            or (cluster[1] == "K" and cluster[2] == "T")
            or (cluster[1] == "K" and cluster[2] == "S")
            or (cluster[1] == "S" and cluster[2] == "T")
        ):
            return True
        ...
        # 3-phone clusters beginning with R, e.g., carts, worst
            ...
            (cluster[1] == "P" and cluster[2] == "T")
            or (cluster[1] == "P" and cluster[2] == "S")
            or (cluster[1] == "M" and cluster[2] == "TH")
            or (cluster[1] == "T" and cluster[2] == "S")
            or (cluster[1] == "K" and cluster[2] == "T")
            or (cluster[1] == "S" and cluster[2] == "T")
        ):
        ...
        elif cluster[0] == "N" and (cluster[1] == "D" and cluster[2] == "TH"):
            return True
        ...
        # 3-phone clusters beginning with NG, e.g., angst
            ...
            or (cluster[1] == "K" and cluster[2] == "S")
            or (cluster[1] == "K" and cluster[2] == "TH")
            or (cluster[1] == "S" and cluster[2] == "T")
        ):
            return True

        # 3-phone clusters beginning with K, e.g., sixth
        elif (cluster[0] == "K" and cluster[1] == "S" and cluster[2] == "TH") or (
    ...
    if length == 2:
        if (
            (cluster[1] == "S" and cluster[0] in S_EXTENDED_CODAS)
            or (cluster[1] == "Z" and cluster[0] in Z_EXTENDED_CODAS)
            or (cluster[1] == "T" and cluster[0] in T_EXTENDED_CODAS)
            or (cluster[1] == "D" and cluster[0] in D_EXTENDED_CODAS)
        ):
            length -= 1
        ...
        if cluster[0] == "L" and (
            ...
            or cluster[1] in AFFRICATES
            or cluster[1] in {"F", "S", "SH", "TH", "V"}
            or cluster[1] in NASALS.difference(["NG"])
        ):
            return True
        ...
        # 2-phone clusters beginning with nasals, e.g., bent, ink
            ...
            return True
        elif cluster[0] == "N" and cluster[1] in {
            "T",
            "D",
            "CH",
            "JH",
        ...
        # 2-phone clusters beginning with stops, e.g., pact, width
            ...
            or (cluster[0] == "S" and cluster[1] in {"P", "T", "K"})
            or (cluster[0] == "P" and cluster[1] in {"T", "TH", "S", "F"})
            or (cluster[0] == "K" and cluster[1] in {"T", "S", "SH"})
            or (cluster[0] == "T" and cluster[1] in {"S", "TH"})
            or (cluster[0] == "D" and cluster[1] == "TH")
        ):
            return True
